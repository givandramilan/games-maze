...
class Player(GameSprite):
    ...
    def update(self):
        ...
        if packman.rect.x <= win_width-80 and packman.x_speed > 0 :
            self.rect.x += self.x_speed 
        
        if packman.rect.x >= 0 and packman.x_speed < 0:
            self.rect.x += self.x_speed 
        ...
        if packman.rect.y <= win_height-80 and packman.y_speed > 0 :
            self.rect.y += self.y_speed 
        
        if packman.rect.y >= 0 and packman.y_speed < 0:
            self.rect.y += self.y_speed 
        platforms_touched = sprite.spritecollide(self, barriers, False)
        if self.y_speed > 0:
            for p in platforms_touched:
